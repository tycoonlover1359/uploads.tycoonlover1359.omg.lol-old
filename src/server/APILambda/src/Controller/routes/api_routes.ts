...
router.post("/up", async (req: Request, res: Response) => {
    ...
    const uploadedFile = req.files.data as UploadedFile;

    const thumbnail = await sharp(uploadedFile.data).resize(200).toFormat("png").toBuffer();

    res.status(200).contentType("png").end(thumbnail, "binary");
});
